...
def create_app(
    test_config, clubs_list, competitions_list, save_clubs_func, save_comps_func
):
    app = Flask(__name__)
    ...
    def showSummary():
        ...
        try:
            club = [
                club for club in clubs_list if club["email"] == request.form["email"]
            ][0]
    ...
    def purchasePlaces():
        competition = [
            c for c in competitions_list if c["name"] == request.form["competition"]
        ][0]
...
if __name__ == "__main__":
    ...
    app = create_app(
        ...
        save_clubs_func=saveClubs,
        save_comps_func=saveCompetitions,
    )
    app.run(debug=True)
